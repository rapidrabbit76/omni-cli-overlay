...
export function InputBar() {
  ...
  useEffect(() => {
    ...
    document.addEventListener("oco:trigger-slash", handler);
    return () => document.removeEventListener("oco:trigger-slash", handler);
  }, []);
  ...
  const executeCommand = useCallback(
    (cmd: SlashCommand) => {
      switch (cmd.command) {
        ...
        case "/fork":
          if (tab?.sessionId) {
            resumeSession(tab.sessionId, `${tab.title} (fork)`, tab.workingDirectory).catch(() =>
              addSystemMessage("Failed to fork current session."),
            );
          } else {
  ...
  const handleSend = useCallback(() => {
    ...
    if (hasPrefix && activePrefix!.type === "command" && !prompt) {
      const commandMeta = getFilteredCommandsWithExtras(activePrefix!.value, []).find(
        (c) => c.command === activePrefix!.value,
      );
      if (commandMeta) {
        executeCommand(commandMeta);
        setInput("");
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (showSlashMenu) {
      const filtered = getFilteredCommandsWithExtras(slashFilter!, []);
      if (e.key === "ArrowDown" || isCtrlN(e)) {
        e.preventDefault();
        setSlashIndex((i) => (i + 1) % filtered.length);
        return;
      }
      if (e.key === "ArrowUp" || isCtrlP(e)) {
        e.preventDefault();
    ...
    if (showSkillMenu) {
      ...
      if (e.key === "Tab" || (e.key === "Enter" && !e.shiftKey)) {
        e.preventDefault();
        handleSkillSelect(filteredSkills[skillIndex]);
        return;
      }
      if (e.key === "Escape") {
        e.preventDefault();
    ...
    if (e.key === "Escape" && !showSlashMenu && !showSkillMenu) {
      if (activePrefix) {
        setActivePrefix(null);
        return;
      }
                  ...
                  >
                    <button
                      ...
                      style={{
                        ...
                        color: isConnecting ? colors.textTertiary : colors.textSecondary,
                      }}
                      title="Voice input (or hold Option)"
                    >
                      <Microphone size={16} />
                    </button>
                  </motion.div>
