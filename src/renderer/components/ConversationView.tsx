...
export function ConversationView() {
  ...
  const scrollTrigger = `${msgCount}:${lastMsg?.content?.length ?? 0}:${queuedCount}`
...
function ImageCard({ src, alt, colors }: { src?: string; alt?: string; colors: ReturnType<typeof useColors> }) {
  ...
  const open = () => { if (src) window.oco.openExternal(String(src)) }
  ...
  const markdownComponents = useMemo(() => ({
    ...
    a: ({ href, children }: any) => (
      <button
        ...
        onClick={() => {
          if (href) window.oco.openExternal(String(href))
        }}
      >
        {children}
      </button>
...
// ─── Tool Group (collapsible timeline — Codex style) ───
